        ...
        {items.length === 0 ? (
          ...
          items.map((n, i) => (
            <TableRow key={i} className="align-middle h-[3.083rem]">
              <TableCell className={`w-12 px-[0.833rem] pb-[0.667rem] ${i === 0 ? "pt-0" : "pt-[0.667rem]"}`}>
                <Chip tone={n.tone ?? "danger"}>{(n.tone ?? "danger") === "warn" ? "주의" : "경고"}</Chip>
              </TableCell>

              <TableCell className={`px-[0.833rem] pb-[0.667rem] ${i === 0 ? "pt-0" : "pt-[0.667rem]"}`}>
                <div className="min-w-0 truncate">{n.text}</div>
              </TableCell>

              <TableCell className={`text-right text-xs text-gray-500 whitespace-nowrap px-[0.833rem] pb-[0.667rem] ${i === 0 ? "pt-0" : "pt-[0.667rem]"}`}>
                {n.time ?? "-"}
              </TableCell>
            </TableRow>
          ))
